...
class BridgeConfig extends BaseExtension {
    constructor(zigbee, mqtt, state, publishEntityState) {
        ...
        this.deviceReload= this.deviceReload.bind(this);
        ...
        this.supportedOptions = {
            ...
            'device_reload': this.deviceReload,
        };
    }
    ...
    async deviceReload(topic, message) {
        const entity = this.zigbee.resolveEntity(message.trim());
        const device = entity.device;

        try {
            logger.info(`Removing entity '${entity.settings.ID}' from state`);
            this.state.remove(entity.settings.ID);
            logger.info(`Clearing device '${device.ieeeAddr}' meta`);
            device.meta = {};

            logger.info(`Updating last seen for device '${device.ieeeAddr}'`);
            device.updateLastSeen();

            logger.info(`Interview '${device.ieeeAddr}' start`);
            await device.interview();
            logger.info(`Succesfully interviewed '${device.ieeeAddr}'`);
        } catch (error) {
            logger.error(`Device reload failed for '${entity.settings.ID} with error '${error}'`);
        }
    }
